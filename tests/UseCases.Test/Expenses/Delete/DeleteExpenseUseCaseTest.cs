...
public class DeleteExpenseUseCaseTest
{
    ...
    public async Task Success()
    {
        ...
        var expense = ExpenseBuider.Build(loggedUser);
    ...
    private DeleteExpenseUseCase CreateUseCase(User user, Expense? expense = null)
    {
        ...
        var readOnlyRepository = new ExpenseReadOnlyRepositoryBuilder().GetById(user, expense).Build();
        var unitOfWork = UnitOfWorkBuilder.Build();
        var loggedUser = LoggedUserBuilder.Build(user);

        return new DeleteExpenseUseCase(repository, readOnlyRepository, unitOfWork, loggedUser);
    }
}
